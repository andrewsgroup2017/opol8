<template>
  <div class="chat-log" ref="chatLogContainer">
    <message-bubble v-for="message in messages" :key="message.key" :time="message.time" :text="message.text" :who="message.who"></message-bubble>
...
export default {
  ...
  computed: {
    messages () {
      ...
      return this.$store.state.chat.chatMessages[this.$store.state.currentChat]
    },
  },
  created () {
    ...
    EventBus.$on('typing-start', (chatKey) => {
      if (this.$store.state.chat.currentChat === chatKey) {
